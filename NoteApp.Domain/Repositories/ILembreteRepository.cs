...
public interface ILembreteRepository
{
    ...
    public Task<List<Lembrete>> ObterLembretes(int take, int skip, Guid? userId, CancellationToken cancellationToken = default);
    public Task<List<Lembrete>> ObterLembretesComVencimentoHoje(int take, int skip, Guid? userId, CancellationToken cancellationToken = default);
